...
# def combo_loss(y_true, y_pred, alpha=0.5, beta = None):     ##### check high weight to mass layer (0)
    ...
    if alpha is not None:
        ...
        print(f'cross_entropy: {cross_entropy} | dice : {dice}')
    else:
...
#          Focal loss          #
################################
def focal_loss(y_true, y_pred, alpha=None, gamma_f=2.):
...
################################
def symmetric_focal_loss(y_true, y_pred, delta=0.7, gamma=2.):
    """
    Parameters
    ----------
    delta : float, optional
...
#################################
def symmetric_focal_tversky_loss(y_true, y_pred, delta=0.7, gamma=0.75):
...
###########################################
def sym_unified_focal_loss(y_true,y_pred, weight=0.5, delta=0.6, gamma=0.5):
    ...
    symmetric_ftl = symmetric_focal_tversky_loss(y_true, y_pred, delta=delta, gamma=gamma)
